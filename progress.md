...
- G4 沿用已确认的 v04 视觉与产品事实，声音替换为本地 FFmpeg 程序合成底床和 16 个动作 cue；正式 composition 不再引用内部参考混音。
- 生成并校验 1080p 有声版、无音轨静音版、8.49 MB GitHub MP4 与 6.24 MB README GIF；有声正式版 `-15.82 LUFS / -1.20 dBTP`，完整解码、黑帧和确定性检查通过。
- 在受保护分支 `codex/publish-phrio-g4-film` 与 PR #10 中接入中英 README：默认中文首页自动播放静音完整预览，点击进入 1080p 有声版；英文首页使用同一媒体和同等证据边界。
- 宣传片只使用受控 Fixture 产品画面，不新增真实设备、模型质量、OpenAI、目标 Mac 或公共二进制分发证据；三类证据继续分开记录。
