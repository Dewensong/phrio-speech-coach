...
## 2026-08-13：先减少启动阻力、展示本地事实，再优化 GitHub 转化

- 产品选择：本轮只做两个用户立刻可见且可独立回退的增量。首页场景灵感解决
  “不知道练什么”，本地练习轨迹解决“练过什么看不出来”；两者复用既有自由练习
  和记录协议，不新增 Session 状态、云端数据或训练结果承诺。
- 计量边界：练习轨迹只能统计已经保存在本机的事实，包括 Session、两次表达、
  已选焦点和留存音频。不得加入总分、能力等级、人格判断、连续打卡或输赢文案，
  也不得把数量增长描述成表达质量提升。
- 传播判断：GitHub 已有少量 Clone、但没有可见访问与 Star，优先改造首次访问者的
  理解和动作路径。中英首页都采用“价值主张 → 静态主视觉 → 宣传片 / 运行 / Star
  → 方法 → 能力 → 隐私 → 路线图”，避免首屏先加载大 GIF；Star CTA 只用于关注
  进展，不以解锁功能、虚假人数或承诺公共包进行诱导。
- 发布边界：维护者未加入 Apple Developer Program。Internal Alpha 源码可继续
  公开，CI ad-hoc 包只作证据；在 Developer ID、公证和非构建目标 Mac 验收前，
  GitHub 首页与外部传播都必须明确“暂无普通用户公共安装包”。
- 增长边界：README 和仓库元数据只能提高到访后的理解与转化，不能保证快速涨星。
  真正增长还需要维护者批准并执行一次带宣传片、明确受众和单一链接的外部分发；
  不购买 Star、不互刷、不制造虚假使用者或伪造训练效果。
